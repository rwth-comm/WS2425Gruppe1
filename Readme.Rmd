...
df %>% 
  ...
  group_by(Gender) %>% 
  summarise(Mean = mean(BI_A, na.rm = TRUE), SE = plotrix::std.error(BI_A, na.rm = TRUE)*1.96) %>% 
ggplot() +
  aes(x = Gender, y = Mean, ymin = Mean - SE, ymax = Mean + SE) +
  geom_point(fill = aachen_color("blue50")) +
  labs(
    x = "Gender",
    y = "Nutzungsintention",
    title = paste0("Einfluss von Geschlecht auf Nutzungsintention n = (", nrow(df), ")"),
    subtitle = "Balkendiagramm für den Einfluss des Geschlechts
    auf die Nutzungsintention eines KI-Dienstes für 
    Gebührenbescheide in der Kommunalverwaltung",
    caption = " "
  ) +
  theme_minimal()
...
df %>% 
  filter(Gender == "Männlich" | Gender == "Weiblich") %>% 
  group_by(Gender) %>% 
  summarise(Mean = mean(TIA_A, na.rm = TRUE), SE = plotrix::std.error(TIA_A, na.rm = TRUE)*1.96) %>% 
ggplot() +
  aes(x = Gender, y = Mean, ymin = Mean - SE, ymax = Mean + SE) +
  geom_point(fill = aachen_color("blue50")) +
  labs(
    x = "Gender",
    y = "Vertrauen",
    title = paste0("Einfluss von Geschlecht auf Vertrauen n = (", nrow(df), ")"),
    subtitle = "Balkendiagramm für den Einfluss des Geschlechts
    auf das Vertrauen eines KI-Dienstes für 
    Gebührenbescheide in der Kommunalverwaltung",
    caption = " "
  ) +
  theme_minimal()
