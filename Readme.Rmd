...
ggplot() +
  aes(x = TIA_B, weight = Mean, ymin = Mean - SE, ymax = Mean + SE) +
  geom_bar(fill = aachen_color("blue50")) +
  geom_errorbar( width = 0.2) +
  labs(
    x = "Neigung, in Technik zu vertrauen",
...
df %>% 
   group_by(BI_B) %>% 
  summarise(Mean = mean(BI_C, na.rm = TRUE), SE = plotrix::std.error(BI_C, na.rm = TRUE)*1.96) %>% 
ggplot() +
  aes(x = BI_B, weight = Mean, ymin = Mean - SE, ymax = Mean + SE) +
  geom_bar(fill = aachen_color("blue50")) +
  geom_errorbar( width = 0.2) +
  labs(
